...
public class CTakesOWLVisitor extends OWLOntologyWalkerVisitor
{
    ...
    private static class Phenotype
    {
        ...
        {
            List<IndexableField> doc = new ArrayList<>(this.synonyms.size() + 3);
            doc.add(new StringField(ID_IDX_NAME, this.id, Store.YES));
            doc.add(new TextField(LABEL_IDX_NAME, this.label, Store.YES));
            String joinedSynonyms = StringUtils.join(this.synonyms, System.lineSeparator());
            String text = (this.definition.trim() + System.lineSeparator() + joinedSynonyms
                + System.lineSeparator() + this.label).trim();
    ...
    {
        super(walker);
        this.ontology = walker.getOntology();
        this.documents = new HashMap<>(INITIAL_MAP_SIZE);
    }
    ...
    {
        Phenotype phenotype = this.documents.get(id);
        if (phenotype == null) {
            phenotype = new Phenotype(id);
            this.documents.put(id, phenotype);
        }
        return phenotype;
    }
    ...
    private boolean isPhenotypicAbnormality(Phenotype phenotype, Map<String, Boolean> abnormalities)
    {
        ...
        if (isIt == null) {
            ...
            for (String parentId : phenotype.parents) {
                Phenotype parent = this.documents.get(parentId);
                if (parent != null) {
    ...
    {
        List<List<IndexableField>> retval = new ArrayList<>(this.documents.size());
        Map<String, Boolean> abnormalities = new HashMap<>(this.documents.size());
        abnormalities.put(ABNROMALITY_ROOT_ID, true);
        for (Phenotype phenotype : this.documents.values()) {
