...
/**
 * Permutes the phrase given, allowing the first token to move around (as opposed to
 * FirstTokenPermutationImpl which doesn't).
 * Written by the Mayo Clinic for apache ctakes, adapted to prevent the first token from
 * being fixed.
 *
 * @version $Id$
 */
public class PTTokenPermutation extends FirstTokenPermutationImpl
{
    ...
    public PTTokenPermutation(final DictionaryEngine firstTokenDictEngine,
        ...
        ivFirstTokenDictEngine = firstTokenDictEngine;
        ivPhrBuilder = phraseBuilder;
        ivTextMetaFieldNames = textMetaFieldNames;

        ivMaxPermutationLevel = maxPermutationLevel;
        ivPermCacheMap = new HashMap<>(maxPermutationLevel);
        for (int i = 0; i <= maxPermutationLevel; i++) {
            final List<List<Integer>> permList = PermutationUtil.getPermutationList(i);
            ivPermCacheMap.put(i, permList);
        }
    }
            ...
            final Map<String, List<LookupAnnotation>> contextMap) throws Exception
    {
        ...
        for (int currentIndex = 0; currentIndex < lookupTokenList.size(); currentIndex++) {
            ...
            if (wEndOffset == -1) {
                ivLogger.debug("Window size set to max perm level.");
            ...
            if (endLookupTokenList.isEmpty()) {
                ivLogger.debug(String.format(INVALID_WINDOW_FORMAT, currentIndex, wEndOffset));
                continue;
            }
    ...
    private Map<String, Set<MetaDataHit>> getNamedMetaDataHits(final Collection<MetaDataHit> firstTokenHits)
    {
        ...
        for (MetaDataHit firstTokenHit : firstTokenHits) {
            for (String name : ivTextMetaFieldNames) {
                ...
                if (text != null) {
                    ...
                    if (ivLogger.isDebugEnabled()) {
                        ivLogger.debug("MetaField " + name + " contains no data.");
                    }
                }
            }
        }
    ...
    {
        if (wLookupTokenList.size() - 1 > ivMaxPermutationLevel) {
            ivLogger.debug("Beyond permutation cache size.");
        ...
        final String[] firstWordPhrases = ivPhrBuilder.getPhrases(singleTokenList);
        ...
        final List<List<Integer>> permutationList = ivPermCacheMap.get(permutationIndex);
        for (List<Integer> permutations : permutationList) {
            ...
            final String[] lookupTokenPhrases = ivPhrBuilder.getPhrases(tempLookupTokens);
    ...
    private List<LookupAnnotation> getWindowAnnotations(final Map<String, List<LookupAnnotation>> contextMap)
    {
        ...
        if (list == null || list.isEmpty()) {
            ivLogger.debug("No context window annotations.");
            return Collections.emptyList();
        }
        return list;
    }
            ...
            final int startOffset, final int endOffset)
    {
        ...
        if (startLookupTokenList.isEmpty() || endLookupTokenList.isEmpty()) {
            ivLogger.debug(String.format(INVALID_WINDOW_FORMAT, startOffset, endOffset));
            return -1;
        }
            ...
            final Map<Integer, List<LookupAnnotation>> wEndOffsetMap)
    {
        ...
        for (LookupAnnotation tempLookupAnnot : startCandidateSet) {
            if (largestWindowAnnot == null || tempLookupAnnot.getLength() > largestWindowAnnot.getLength()) {
                ...
                if (ltCount <= ivMaxPermutationLevel && ltCount > 0) {
                    largestWindowAnnot = tempLookupAnnot;
                } else if (ivLogger.isDebugEnabled()) {
                    ivLogger.debug("Window size of " + ltCount
                            + " exceeds the max permutation level of "
                            + ivMaxPermutationLevel + ".");
                }
            }
        }
    ...
    private int getFixedWindowEndOffset(final int tokenIdx, final LookupToken lt, final List<LookupToken> ltList)
    {
        ...
        final int count = Math.min(tokenIdx + ivMaxPermutationLevel, ltList.size());
        if (count <= 0) {
            return 0;
        }
     ...
     */
    private static <T extends LookupAnnotation> Map<Integer, List<T>>
        getMultipleStartOffsetMap(final List<T> lookupAnnotList)
    {
     ...
     * @return map of integers and lookup annotations
     */
    private static <T extends LookupAnnotation> Map<Integer, T>
        getSingleEndOffsetMap(final List<T> lookupAnnotList)
    {
     ...
     */
    private static <T extends LookupAnnotation> Map<Integer, List<T>>
        getMultipleEndOffsetMap(final List<T> lookupAnnotList)
    {
        final Map<Integer, List<T>> m = new HashMap<>();
        for (T lookupAnnotation : lookupAnnotList) {
            final Integer key = lookupAnnotation.getEndOffset();
    ...
    private Collection<MetaDataHit> getFirstTokenHits(final LookupToken firstLookupToken) throws Exception
    {
        ...
        final String[] phrases = ivPhrBuilder.getPhrases(singleTokenList);
        final List<MetaDataHit> metaDataHits = new ArrayList<>();
        for (String phrase : phrases) {
            final Collection<MetaDataHit> phraseMetaDataHits = ivFirstTokenDictEngine.metaLookup(phrase);
