...
@PrepareForTest(BioLarkResourceUtils.class)
public class BioLarkResourceUtilsTest
{
    /**
     * Root directory of biolark resource files
     */
    private final static File biolarkRoot = new File(
        BioLarkResourceUtils.ROOT_DIRECTORY);

    /**
     * Biolark properties file
     */
    private final static File biolarkProperties = new File(
        BioLarkResourceUtilsTest.biolarkRoot,
        BioLarkResourceUtils.PROPERTIES_FILENAME);
    ...
    {
        PowerMockito.spy(BioLarkResourceUtils.class);

        // mock extracting files
        PowerMockito.doNothing().when(BioLarkResourceUtils.class);
        BioLarkResourceUtils.extractArchive(Matchers.any(File.class),
